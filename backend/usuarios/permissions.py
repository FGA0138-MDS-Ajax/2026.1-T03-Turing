...
class IsGoStudyAluno(permissions.BasePermission):
    """
    permissão que concede acesso apenas a usuários autenticados cujo perfil possua o tipo 'aluno'
    """
    def has_permission(self, request, view):
        if not request.user or not request.user.is_authenticated:
            return False
        ...
        try:      
            return request.user.tipo == 'aluno'
        except AttributeError:
            return False
